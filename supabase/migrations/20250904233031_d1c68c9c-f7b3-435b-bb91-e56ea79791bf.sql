-- Remove existing admin users (keep only the church admin if exists)
...
DROP POLICY IF EXISTS "Block profile updates" ON public.profiles;
...
DROP POLICY IF EXISTS "Public can view general settings" ON public.site_settings;
